...
function findSeeds() {
  ...
  for (let x = step; x < img.width - step; x += step) {
    for (let y = step; y < img.height - step; y += step) {
      ...
      const _whitePenalty = parseFloat(document.getElementById('inp-white-intensity')?.value || 0) * 0.35;
      if (random(100) < dt + fc * 100 - _whitePenalty) {
        allSeeds.push({
...
function getPointColor(pt) {
  ...
  if (pt.slot?.type === 'yellow') return meshYellowColor;
  if (pt.slot?.type === 'white')  return '#F0F0F0';
...
function _makeMeshCanvas(w, h) {
  ...
  const _yellowInt = parseFloat(document.getElementById('inp-yellow-intensity')?.value || 0) / 300;
  const _whiteInt  = parseFloat(document.getElementById('inp-white-intensity')?.value || 0) / 200;
  const pts = meshPoints.map(pt => {
    const rgb = _hexToRGB(getPointColor(pt));
    let w = pt.weight || 1.0;
    if (pt.slot?.type === 'yellow') w *= _yellowInt;
    if (pt.slot?.type === 'white')  w *= _whiteInt;
    return { x: pt.x, y: pt.y, r: rgb[0], g: rgb[1], b: rgb[2], weight: w };
...
function _updateTextureCounts() {
  const yc = document.getElementById('yellow-count-display');
  const wc = document.getElementById('white-count-display');
  if (yc) yc.textContent = meshPoints.filter(p => p.slot?.type === 'yellow').length;
  if (wc) wc.textContent = meshPoints.filter(p => p.slot?.type === 'white').length;
}

function addYellowPoint() {
  meshPoints.push({ id: _meshNextId++, x: 0.15 + Math.random() * 0.7, y: 0.15 + Math.random() * 0.7, slot: { type: 'yellow' }, weight: 1.0 });
  _meshEditorUpdate();
}
function removeLastYellowPoint() {
  const last = [...meshPoints].reverse().find(p => p.slot?.type === 'yellow');
  if (!last) return;
  meshPoints = meshPoints.filter(p => p.id !== last.id);
  if (meshSelectedId === last.id) meshSelectedId = meshPoints[0]?.id ?? null;
  _meshEditorUpdate();
}
function addWhitePoint() {
  meshPoints.push({ id: _meshNextId++, x: 0.15 + Math.random() * 0.7, y: 0.15 + Math.random() * 0.7, slot: { type: 'white' }, weight: 1.0 });
  _meshEditorUpdate();
}
function removeLastWhitePoint() {
  const last = [...meshPoints].reverse().find(p => p.slot?.type === 'white');
  if (!last) return;
  meshPoints = meshPoints.filter(p => p.id !== last.id);
  if (meshSelectedId === last.id) meshSelectedId = meshPoints[0]?.id ?? null;
  _meshEditorUpdate();
...
function _applySettingsKeepYW(data) {
  const savedYellowPts = meshPoints.filter(p => p.slot?.type === 'yellow').map(p => ({ ...p }));
  const savedWhitePts  = meshPoints.filter(p => p.slot?.type === 'white').map(p => ({ ...p }));
  const savedYellowInt = document.getElementById('inp-yellow-intensity')?.value;
  const savedWhiteInt  = document.getElementById('inp-white-intensity')?.value;
  const savedYellowCol = document.getElementById('inp-yellow-color')?.value;

  applySettings(data);

  savedYellowPts.forEach(p => meshPoints.push({ ...p, id: _meshNextId++ }));
  savedWhitePts.forEach(p => meshPoints.push({ ...p, id: _meshNextId++ }));

  if (savedYellowInt !== undefined) {
    const el = document.getElementById('inp-yellow-intensity');
    if (el) { el.value = savedYellowInt; el.dispatchEvent(new Event('input')); }
  }
  if (savedWhiteInt !== undefined) {
    const el = document.getElementById('inp-white-intensity');
    if (el) { el.value = savedWhiteInt; el.dispatchEvent(new Event('input')); }
  }
  if (savedYellowCol) {
    meshYellowColor = savedYellowCol;
    const el = document.getElementById('inp-yellow-color');
    if (el) el.value = savedYellowCol;
  }
  _meshEditorUpdate();
}
...
function initSettingsToggle() {
  ...
  btnTexture.addEventListener('click', () => {
    _applyView('texture');
    _applySettingsKeepYW(TEXTURE_SETTINGS);
  });
  btnText.addEventListener('click', () => {
    _applyView('text');
    _applySettingsKeepYW(DEFAULT_SETTINGS);
  });

  // Intensity / color change → refresh preview
  ...
  document.getElementById('inp-yellow-color')?.addEventListener('input', () => {
    meshYellowColor = document.getElementById('inp-yellow-color').value;
    if (currentMode === 'gradient') _scheduleGradientPreview(50);
  });

  // Count buttons
  document.getElementById('btn-yellow-add')?.addEventListener('click', addYellowPoint);
  document.getElementById('btn-yellow-remove')?.addEventListener('click', removeLastYellowPoint);
  document.getElementById('btn-white-add')?.addEventListener('click', addWhitePoint);
  document.getElementById('btn-white-remove')?.addEventListener('click', removeLastWhitePoint);
}
